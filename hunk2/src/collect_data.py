...
def collect_trade_history(cfg: Config) -> None:
    ...
    Sparas i: DATA_AREA_ROOT_DIR/trades/<currency>_<quote_asset>_trades_<timestamp>.json
    """
    base = Path(cfg.data_area_root_dir)
    ...
    try:
        ...
        for cur in cfg.currencies:
            for quote_asset in cfg.allowed_quote_assets:
                ...
                if trades:  # Only save if there are trades
                    out_file = out_root / f"{cur}_{quote_asset}_trades_{_timestamp()}.json"
                    with out_file.open("w", encoding="utf-8") as fh:
